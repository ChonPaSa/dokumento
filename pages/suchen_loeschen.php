...
$text.= '<form action="?page=suchen" method="post">
			...
			<input type="text" name="docname" value="'.$document["Name"].'" disabled />
			...
			$text.='</select><br />
			<label for="abs_emp">Absender / Empfänger: </label><textarea name="abs_emp" disabled>'.$document["absender"].'</textarea><br />		
			<label for="ablage">Ablageort: </label><input type="text" name="ablage" value ="'.$document["ablageort"].'" disabled /><br />';						
			$text.='<input type="hidden" name="dokNr"  value="'.$_GET["dokNr"].'"/>';
			$text.='<input type="submit" name="dokument_loeschen" value="Bestätigen">
